...
export function ReportGeneration() {
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      const blob = await logsService.generateTechSupportReport(format, filters);

      // Create download link
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `techsupport_report_${Date.now()}.${format === "pdf" ? "pdf" : "csv"}`;
  ...
  return (
    <div className="space-y-6">
      <Card title="Generate Tech Support Report">
        <p className="text-sm text-gray-600 mb-6">
          Generate comprehensive reports including all access logs and system logs for troubleshooting.
