...
export function TechSupportAccessLogs() {
  ...
  const columns = [
    ...
    {
      header: "Timestamp",
      accessor: (log: LogDisplay) => log.timestamp.toLocaleString(),
    },
    {
      header: "Method",
      accessor: (log: LogDisplay) => (
        <div>
          {log.source === "tuya" ? (
            ...
            <span className="text-sm text-gray-600">{log.labName}</span>
          )}
        </div>
      ),
    },
      ...
      {activeTab === "access" && (
        <Card title="Access Logs">
          <div className="mb-4 p-3 bg-gray-50 rounded-lg">
            ...
            <ul className="text-sm text-gray-600 mt-2 list-disc list-inside space-y-1">
              <li>User who accessed the door lock</li>
              <li>Timestamp of access</li>
              <li>Access method and status (includes Tuya unlocking records)</li>
