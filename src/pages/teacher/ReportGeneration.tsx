...
export function ReportGeneration({ teacherId }: ReportGenerationProps) {
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      const blob = await logsService.generateTeacherReport(format, filters);

      // Create download link
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `teacher_report_${Date.now()}.${format === "pdf" ? "pdf" : "csv"}`;
  ...
  return (
    <div className="space-y-6">
      <Card title="Generate My Report">
        <p className="text-sm text-gray-600 mb-6">
          Generate a report of your personal access logs and activities. Reports can be exported as PDF or CSV format.
        </p>
